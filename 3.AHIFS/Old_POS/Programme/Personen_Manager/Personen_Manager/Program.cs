...
namespace Personen_Manager
{
    class Program
    {
        ...
        {
            PersonenManagerSaverBinary SaverBinary = null;
            PersonenManagerSaverCSV SaverCSV = null;
            PersonenManager personmanager = null;
            try
            {
                string FileTyp;
                string FileName;
                char AuswahlSave = ' ';
                personmanager = new PersonenManager();
                Person person = null;
                ...
                switch (AuswahlPerson)
                {
                    case 'M':
                        ...
                        ErstellenVonMitarbeiter(Anzahl, personmanager);
                        break;

                    case 'V':
                        Anzahl = GetAValidNumber(1, 10, "Wie viele Vorgesetzte wollen Sie erstellen?");
                        ErstellenVonVorgesetzten(Anzahl, personmanager);
                ...
                {
                    case 'B':
                        SaverBinary = new PersonenManagerSaverBinary();
                        Console.WriteLine("Wie soll das File heißen: ");
                        FileTyp = ".txt";
                        FileName = Console.ReadLine();
                        SaverBinary.Filename = FileName + FileTyp;
                        SaverBinary.Save(personmanager);
                        personmanager = SaverBinary.Load();
                        personmanager.PrintAll();
                        break;

                    case 'C':
                        SaverCSV = new PersonenManagerSaverCSV();
                        Console.WriteLine("Wie soll das File heißen: ");
                        FileTyp = ".csv";
                        FileName = Console.ReadLine();
                        SaverCSV.Filename = FileName + FileTyp;
                        SaverCSV.Save(personmanager);
                        personmanager = SaverCSV.Load();
                        personmanager.PrintAll();
                        break;
                }
                #endregion
