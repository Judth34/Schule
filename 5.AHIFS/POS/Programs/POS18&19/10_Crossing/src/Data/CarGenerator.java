...
public class CarGenerator extends Task<String>{
    ...
    public CarGenerator(long timeBetweenArrival, long waitingTime, ObservableList<String> obsList) {
    ...
    protected String call() {
        ...
        while(!this.isEnd){
            ...
            int index = ThreadLocalRandom.current().nextInt(0, 1 + 1);
            int index2 = ThreadLocalRandom.current().nextInt(0, 3 + 1);
            while(index == index2)
                index2 = ThreadLocalRandom.current().nextInt(0, 3 + 1);

            Car car = new Car("Car-" + countCars, Place.values()[index], Place.values()[index2], 2000, new Semaphore(1), new Semaphore(1));
